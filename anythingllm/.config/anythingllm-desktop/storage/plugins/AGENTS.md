...
5. **Строго `generic-openai` с дефисом:** Идентификатор провайдера в AnythingLLM строго `generic-openai`. Формат `generic_openai` вызывает падение `AgentHandler.checkSetup()`.
6. **Пополняйте базу грабель:** Все архитектурные особенности, баги и нюансы связки AnythingLLM + Ollama + Headroom обязательно документируются и пополняются в [`docs/headroom_anythingllm_troubleshooting.md`](file:///data/projects/dotfiles/docs/headroom_anythingllm_troubleshooting.md).
7. **Компактность схем инструментов (SLM Tool Ecology):** Компактные локальные модели (Gemma 4B, Qwen 4B) не выдерживают 20+ громоздких инструментов официального `server-filesystem`. Держите набор инструментов минималистичным (до 6 инструментов), с плоскими схемами и понятными описаниями.
8. **Очистка и лимитирование вывода инструментов на стороне сервера:** Никогда не отдавайте из MCP-инструментов файлы целиком или неограниченные простыни текста. Очищайте YAML frontmatter, извлекайте выдержки (первые 200–250 символов) и ограничивайте число результатов (`head -n 25`), чтобы не раздувать контекст выше 3–4 KB.
9. **Защита инструментов от сжатия в Headroom Proxy:** При использовании Headroom с локальными SLM всегда защищайте результаты инструментов флагами `--protect-tool-results` и `--no-ccr`. Иначе Headroom заменит вывод на `<<ccr:...>>`, который модель воспримет как буквальное название файла.
10. **Стемминг русского и английского языков:** Поиск по файлам (`search_files`) обязан поддерживать отсечение падежных флексий и суффиксов, иначе запросы в косвенных падежах (например, «лидерство») пропустят заметки с заголовками в других формах («Как быть лидером»).
