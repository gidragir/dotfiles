                ...
                NText {
                    ...
                    text: {
                        if (root.filteredItems.length === 0) {
                            if (root.filterText) return root.pluginApi?.tr("provider.noResults") || "No results";
                            return "";
                        }
                        var count = root.filteredItems.length;
                        if (root.filterText && root.showMatchCount) {
                            var total = root.dmenuState ? root.dmenuState.items.length : count;
                            var s = root.pluginApi?.trp("provider.filteredResultsCount", count, { filtered: count, total: total });
                            if (!s || s.startsWith("!")) {
                                return count + " of " + total + " · " + count + (count === 1 ? " result" : " results");
                            }
                            return s;
                        }
                        var res = root.pluginApi?.trp("provider.resultsCount", count);
